...
import {
  ...
  createApiKey,
  updateApiKey,
} from "@/lib/supabase/queries/apiKeys";
...
export interface APIKeyState {
  ...
  validationStatus: Record<
    string,
    "valid" | "invalid" | "checking" | "unknown"
  >;
...
export function APIKeyProvider({ children }: { children: React.ReactNode }) {
  ...
  const validateApiKeyImpl = async (
    serviceName: string,
    apiKey: string,
  ): Promise<boolean> => {
    if (!apiKey) {
  ...
  const validateApiKey = useCallback(
    async (serviceName: string, apiKey?: string): Promise<boolean> => {
  ...
  const saveApiKey = useCallback(
    async (serviceName: string, apiKey: string): Promise<void> => {
      if (!user) {
        setState((prev) => ({
          ...prev,
      ...
      try {
        ...
        setState((prev) => {
          ...
          if (process.env[ENV_VAR_MAPPING[serviceName]]) {
            newApiKeys[serviceName] =
              process.env[ENV_VAR_MAPPING[serviceName]] || "";
  ...
  const hasValidApiKeyFor = useCallback(
    (serviceName: string): boolean => {
      ...
      if (
        state.usingEnvVars[serviceName] &&
        state.validationStatus[serviceName] === "unknown"
      ) {
        return true;
      }
