...
const clientInstances: Record<ServiceName, AIClient> = {} as Record<
  ServiceName,
  AIClient
>;
...
export function getAIClient(service: ServiceName, apiKey?: string): AIClient {
  ...
  if (!apiKey) {
    console.log(
      `No API key provided, checking environment variables for ${service}`,
    );
...
export async function initializeClient(
  ...
  console.log(
    `Initializing client for ${serviceName}, API key provided: ${apiKey ? "yes" : "no"}`,
  );
  ...
  if (!apiKey) {
    ...
    throw new Error(
      `${serviceName.charAt(0).toUpperCase() + serviceName.slice(1)} API key is required`,
    );
  }
